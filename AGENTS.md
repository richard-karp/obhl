...
- **`ACCESS_CONTROL_HANDOFF.md`** — who can do what, and where: the
  ...
  **no RLS half on purpose** — the scorekeeper day restriction behind
  `/manage/tonight`. Read that entry before "fixing" what looks like a missing
  policy, and before adding any page under `/manage/`: a legacy redirect in
  `next.config.ts` silently eats some of those paths.
