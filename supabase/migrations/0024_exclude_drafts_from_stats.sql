-- Standings and stats must ignore draft games.
--
-- These views are `security_invoker`, so under the RLS clients the `public read
-- games` policy (`not is_draft and season_is_public(...)`) was already keeping
-- drafts out. The admin client bypasses RLS, so a manager-only read through the
-- same view saw whatever the view itself allowed — and the view said only
-- `status = 'final'`. No draft can reach `final` today (the generator inserts
-- them as `scheduled`, publishing clears the flag before anything else touches
-- them, and the scoring list filters drafts out), so this changes no current
-- numbers. It stops the guarantee from depending on which client asks.
--
-- Recreated (not altered) because the only change is an added WHERE predicate;
-- column lists are identical, so `create or replace view` is safe — the same
-- reasoning as 0014, whose definitions these are.
...
select
  ...
  g.result_type
from games g
where g.status = 'final' and g.game_type = 'regular' and not g.is_draft
union all
select
  g.id,
  g.season_id,
