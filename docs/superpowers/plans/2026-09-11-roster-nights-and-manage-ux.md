...
- [ ] **Step 7** — `npm test`. `revalidate-paths.test.ts` walks `src/app` and resolves every pattern against the real route tree, so a missed string fails **by name** — trust it over grepping. Then belt-and-braces: `grep -rn "schedule-builder" src/` must return no href and no `revalidatePath` string, only the redirect page's own directory.
