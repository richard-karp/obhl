...
|---|---|---|---|
| 6 teams / 1 weeknight / 3 sheets | **13 -> 4** | **28 -> 17** | **25.8 s -> 6.5 s** |
...
production. At 1,000 the 6-team league finishes its restarts (6.5 s) and the
promise holds for it. The 8-team league remains clock-bound at 1,000 (26.3 s);
...
`assignNights` gains `seed?: number` on `AssignOptions`. A *variation* `v` is
generated by running seeds `(v-1)*N + 1 .. v*N` and keeping the best by
`rankSchedule` — the existing lexicographic comparator, which already ends with
the two clustering terms, so clustering is the tiebreaker it never was inside
Phase S.

This is the one place clustering can enter selection cheaply: `rankSchedule` runs
on the **finished** schedule, after the night-order pass, so it sees what the
manager sees.
...
|---|---|---|
| <= 80 | 4 | 6-team league: 4 x 6.5 s ~= 26 s |
| 81-120 | 2 | |
