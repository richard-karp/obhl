...
✅ **`obhl.vercel.app` STAYS, deliberately.** The user's decision, same day: only
two or three people know that host, so there is no redirect to build and no
migration of anyone's bookmarks. It simply keeps working alongside the new name.
⛔ **That makes the alias trap below permanent rather than temporary** — the old
host will never stop answering, so it can never be used as evidence that the move
worked.
...
⛔ **THE TRAP THAT WILL HIDE A MISTAKE: `obhl.vercel.app` KEEPS WORKING — AND
NOW IT KEEPS WORKING ON PURPOSE, FOREVER.** Vercel leaves it as an alias and the
decision above is to keep it, so a stale allow-list entry, a missed env var, or a
link someone bookmarked all keep functioning — and the flow appears healthy right
up until somebody arrives on the new host and gets silently redirected to the old
one. Nothing will ever fail loudly enough to tell you. ⚠️ **Verify from the NEW host in a fresh private window**, not from a tab you
