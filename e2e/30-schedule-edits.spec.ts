...
test.describe("Path 28 — manual schedule edits", () => {
  ...
  test("a manager can trade teams between two games, and totals survive", async ({
    ...
    await page.locator("#tt-x").selectOption({ index: 1 });
    await page.locator("#tt-outx").selectOption({ index: 1 });
    await page.locator("#tt-y").selectOption({ index: 1 });
    await page.locator("#tt-outy").selectOption({ index: 1 });
    await page.getByRole("button", { name: "Trade these two" }).click();

    // Either it traded, or it named the rule it would have broken. Both are
    // correct outcomes for an arbitrary pair; what must NEVER happen is a
    // changed count.
    await expect(
      page
        .getByText("Traded.")
        .or(page.getByText(/would play|cannot play|twice/)),
    ).toBeVisible();
  ...
  test("moving a game to another night is refused, and says why", async ({
    ...
    await expect(page.getByText(/same night/i)).toBeVisible();
  });
