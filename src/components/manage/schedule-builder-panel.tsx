...
import { getPublishState, getScheduleConstraints } from "@/lib/queries/schedule";
import {
  describeConstraint,
  evaluateConstraints,
  forcedByeCredits,
...
import { formatLongDate, formatGameTime, leagueDateKey, leagueTimeKey } from "@/lib/format";
...
export async function ScheduleBuilderPanel({
  ...
  const constraintTeamIndex = new Map(constraintTeamIds.map((id, i) => [id, i]));
  ...
  const credits =
    placed.length > 0
      ...
      : null;
  const spacing = rawSpacing && credits ? presentSpacing(rawSpacing, credits) : rawSpacing;
            ...
            <Card>
              ...
              <CardContent>
                    ...
                    return (
                      <li key={o.id} className="flex items-start gap-2">
                        ...
                        <span className="text-muted-foreground">
                          {source
                            ? describeConstraint(source, constraintNameOf(o.teamId))
                            : o.kind}
                          {o.reason ? ` — ${o.reason}` : ""}
                        </span>
                      </li>
                ...
                <p className="text-muted-foreground mt-2 text-xs">
                  ...
                  generator was asked to do — a request only counts as met if the
                  games below actually show it. Requests are best effort: an even
                  schedule comes first, so a request can be declined rather than
                  bought with an unbalanced season.
                </p>
              </CardContent>
            </Card>
          ) : null}
