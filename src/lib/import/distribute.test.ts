...
const sum = (
  dist: Map<number, { goals: number; assists: number; pim: number }>,
) => {
  let g = 0,
    a = 0,
    pim = 0;
...
const games: DistGame[] = [
  4, 2, 6, 3, 5, 1, 3, 4, 2, 7, 3, 5, 2, 4, 6, 1, 3, 5, 2, 4,
].map((cap) => ({ cap }));
...
describe("distributeStats", () => {
  ...
  it("never records more skater goals in a game than the team scored", () => {
    ...
    perGame.forEach((scored, gi) => {
      expect(scored, `game ${gi} goal overflow`).toBeLessThanOrEqual(
        games[gi].cap,
      );
    });
  });
