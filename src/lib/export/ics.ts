    ...
    .map((g) => {
      ...
      const title =
        g.status === "final"
          ...
          : `${g.away} @ ${g.home}`;
      return {
        // `@obhl` is an opaque namespace, not the league: the UID is subscription identity, and
        // changing it re-adds every event, losing reminders and duplicating downloads.
        uid: `game-${g.id}@obhl`,
        start: [
          d.getUTCFullYear(),
          d.getUTCMonth() + 1,
