...
// The same six-team shape with one manager request on it. Every constraint gates
// the night-order pass off — including a `slot_bias`, which names no night at
// all — so a constrained league gets its clustering broken inside Phase M or not
// at all. Measured 2026-09-09 before the compound pass: worst team 16, against 4
// for the identical unconstrained season above. (16, not the 14 §5 quotes for
// this shape — that is the *unconstrained* season's pre-night-order number, and
// the `slot_bias` costs two more windows on top of it.)
describe("assignNights — ice-time clustering, constrained 6-team season", () => {
  const ts = teams(6);
  const SLOT_TIMES = ["19:00", "20:15", "21:30"];
  const ns = enumerateNights("2026-09-08", {
    weekdays: new Set([2]),
  ...
  // The weakest request in the whole feature: no night index, no slot pin, just
  // a preference ranked below ordinary repeats. It still trips the gate.
  ...
  const { games, report } = assignNights(
    buildBalancedPairings(ts, 23),
    ns,
    ts,
    { constraints },
  );

  // Same re-derivation as the unconstrained block: `scheduledAt` is the only
  // placement fact that persists, so a claim this disagrees with is a quality
  ...
  it("has a constraint set that really does gate the night-order pass off", () => {
    expect(constraints.empty).toBe(false);
    expect(constraints.items[0].unresolved).toBeNull();
  });
  ...
  // ⛔ The same guard the unconstrained block carries. Nothing reorders nights on
  // this path *today* — that is what being gated out of the night-order pass
  // means — but a report that agrees with a `scheduledAt` nobody checked is
  // exactly how `ea62f62`'s no-op passed 364 tests through four review rounds.
  it("moves scheduledAt with nightIndex", () => {
    for (const g of games)
      expect(g.scheduledAt.slice(0, 10)).toBe(ns[g.nightIndex].date);
  });
  ...
  // The three invariants a constraint may never move, plus the spacing guard the
  // compound pass carries: clustering may not be bought with opponent balance or
  // with rematches.
  it("holds opponent balance and rematch spacing while doing it", () => {
    const meetings = new Map<string, number>();
    const perNight = new Map<string, number>();
    for (const g of games) {
      const k = [g.home, g.away].sort().join("|");
      meetings.set(k, (meetings.get(k) ?? 0) + 1);
      const d = g.scheduledAt.slice(0, 10);
  ...
  // clustering re-deals games between nights, which changes what Phase S is
  // handed — so a fix that quietly cost the manager their `slot_bias` would be
  // exactly the silent downgrade the gate above exists to prevent. Measured
  // 2026-09-09 either side of the pass: satisfied both ways, and the team's mean
  // ice-time index is 0.957 against a 1.0 midpoint in both.
  it("does not buy the clustering with the request that was made", () => {
...
// The 23-night fixture above is one point, and `SCHEDULE_HANDOFF.md` §5 records
// that this pass's numbers do not generalise from it: the worst team still falls
// a long way here (25 → 11) but `slotClusterWindows` goes the OTHER way (48 →
// 58), because Phase M optimises periodicity as a proxy and cannot see ice times
// at all. This fixture exists so that divergence has a floor under it — assert
// the worst team and the broken cycle, and deliberately do NOT assert the total.
//
// Both assertions were verified against the pre-pass code (2026-09-09): with
// `breakPeriodicity` off this season reads worst team 25 and 5 distinct
// matchup-sets, so each of them fails without the pass.
describe("assignNights — ice-time clustering, longer constrained 6-team season", () => {
  const ts = teams(6);
  const SLOT_TIMES = ["19:00", "20:15", "21:30"];
  const ns = enumerateNights("2026-09-08", {
    weekdays: new Set([2]),
  ...
  it("schedules the whole season", () => {
    expect(report.unscheduled).toBe(0);
    expect(games.length).toBe(120);
    for (const t of report.gamesPerTeam) expect(t.count).toBe(40);
  });
  ...
  // five-night 1-factorisation cycle is what the pass exists to break, and on 40
  // nights an unbroken one reads exactly 5.
  it("breaks the periodic cycle", () => {
    const byNight = new Map<number, string[]>();
    for (const g of rows) {
      const list = byNight.get(g.nightIndex) ?? [];
      list.push([g.home, g.away].sort().join("-"));
      byNight.set(g.nightIndex, list);
    }
    const distinct = new Set(
      [...byNight.values()].map((v) => v.sort().join(",")),
    );
    expect(distinct.size).toBeGreaterThanOrEqual(10);
  });

  it("holds opponent balance while doing it", () => {
    const meetings = new Map<string, number>();
    const perNight = new Map<number, number>();
    for (const g of rows) {
      const k = [g.home, g.away].sort().join("|");
      meetings.set(k, (meetings.get(k) ?? 0) + 1);
      perNight.set(g.nightIndex, (perNight.get(g.nightIndex) ?? 0) + 1);
    }
    expect(meetings.size).toBe(15);
