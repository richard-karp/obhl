...
- **A ROLE-ONLY GUARD ON A PAGE IS NORMALLY A BUG — `/manage/leagues/new` IS THE
  ...
  page belonging to no league, and it would fail both pages it sweeps.
  (`/set-password` is not among them — it lives at `src/app/set-password/`,
  outside `src/app/manage/` entirely.)
